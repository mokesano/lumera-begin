...
<div id="{$containerId}" class="canvas">
			...
			$('.citation-save-button').each(function() {ldelim}
				$(this).click(function() {ldelim}
					...
					$('#{$containerId}').bind('submitSuccessful', function(e, $updatedElement) {ldelim}
						...
						if($('#citationFormMessages .sep_form_error_list').children().length === 0) {ldelim}
						...
						if (pressedButton === 'citationFormSave') {ldelim}
							if ($('#citationFormMessages').length == 0) {ldelim}
								var formErrorHtml =
									...
									'    <div id="sep_form_errors">'+
									'        <p><span class="sep_form_error">{translate key="submission.citations.editor.details.messages"}:</span></p>'+
									'        <ul class="sep_form_error_list"></ul>'+
									'    </div>'+
									'</div>';
							...
							$('#formErrors .sep_form_error_list').append(messageHtml);
						{rdelim}
	...
	<form class="sep_form" id="editCitationForm" method="post" action="{url op="updateCitation"}" >
			...
			{if $citation->getId()}
				<div id="citationImprovementBlock">
					...
					<div id="citationImprovement">
						...
						<div id="citationImprovementManual" class="sep_controllers_grid">
							<table><tbody>
