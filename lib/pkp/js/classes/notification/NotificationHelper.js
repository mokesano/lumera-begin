...
$.sep.classes.notification = $.sep.classes.notification || {};
...
(function($) {
	...
	$.sep.classes.notification.NotificationHelper = function() {};
	 ...
	 * notifications.
	 *
	 * @param {$.sep.classes.Handler} handler The widget handler that is
	 * handling the notify user event.
	 * @param {HTMLElement} triggerElement The element that triggered the
	 * notify user event.
	 */
	$.sep.classes.notification.NotificationHelper.redirectNotifyUserEvent =
		...
		var $notificationSelector = '.sep_notification';
		...
		if (!(handler instanceof $.sep.controllers.SiteHandler)) {
			if ($(triggerElement).parents('.sep_controllers_grid').length > 0) {
		...
		for (i = 0, length = $pageNotificationElements.length; i < length; i++) {
			...
			// FIXME If we use a class to identify sep widgets, we can avoid
			...
			for (j = 0, parentsLength = $elementParents.length;
					j < parentsLength; j++) {
				handler = $($elementParents[j]).data('sep.handler');
