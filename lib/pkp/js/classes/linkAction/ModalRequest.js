...
(function($) {
	 ...
	 *  request.
	 */
	$.sep.classes.linkAction.ModalRequest =
	 ...
	 * @type {jQuery}
	 */
	$.sep.classes.linkAction.ModalRequest.prototype.$dialog_ = null;
	 ...
	 * @inheritDoc
	 */
	$.sep.classes.linkAction.ModalRequest.prototype.activate =
		...
		var uuid = $.sep.classes.Helper.uuid();
		...
		var linkActionHandler = $.sep.classes.Handler.getHandler($linkActionElement);
		var handlerOptions = $.extend(true,
				{$eventBridge: linkActionHandler.getStaticId()}, modalOptions);
		this.$dialog_ = $('<div id=' + uuid + '></div>').sepHandler(
				modalOptions.modalHandler, handlerOptions);

		// Subscribe to the dialog handler's 'removed' event so that
		// we can clean up.
		var dialogHandler = $.sep.classes.Handler.getHandler(this.$dialog_);
		dialogHandler.bind('sepRemoveHandler',
				$.sep.classes.Helper.curry(this.finish, this));
	 ...
	 * @inheritDoc
	 */
	$.sep.classes.linkAction.ModalRequest.prototype.finish =
			function() {
