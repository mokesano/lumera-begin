...
(function($) {
	 ...
	 */
	$.sep.controllers.PageHandler = function($page, options) {
	...
	$.sep.classes.Helper.inherits(
			$.sep.controllers.PageHandler, $.sep.classes.Handler);
	 ...
	 */
	$.sep.controllers.PageHandler.prototype.redirectToUrl =
	 ...
	 * @private
	 */
	$.sep.controllers.PageHandler.prototype.redirectNotifyUserEventHandler_ =
	 ...
	 * @private
	 */
	$.sep.controllers.PageHandler.prototype.redirectDataChangedEventHandler_ =
		...
		var linkActionHandler = $.sep.classes.Handler.getHandler($sourceLinkElement);
		...
		var $grids = $('.sep_controllers_grid', this.getHtmlElement())
				.has('a[href=' + linkUrl + ']');
	 ...
	 * @private
	 */
	$.sep.controllers.PageHandler.prototype.gridRefreshRequestedHandler_ =
		...
		for (var key in updateSourceElementClasses) {
			if (updateSourceElementClasses[key].search(updateSourceClassString) != -1) {
				...
				if ($targetElements.length > 0) {
					var $grids = $targetElements.find('.sep_controllers_grid');
					if ($grids.length > 0) {
						$grids.each(function() {
							// Keyword "this" is being used here in the
							// context of the grid html element.
