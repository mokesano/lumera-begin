...
(function($) {
	 ...
	 */
	$.sep.controllers.NotificationHandler =
	 ...
	 * @type {Object}
	 */
	$.sep.controllers.NotificationHandler.prototype.options_ = null;
	 ...
	 * @type {Object}
	 */
	$.sep.controllers.NotificationHandler.prototype.trivialTimer_ = null;
	 ...
	 * @private
	 */
	$.sep.controllers.NotificationHandler.prototype.fetchNotificationHandler_ =
			function() {
	 ...
	 * @private
	 */
	$.sep.controllers.NotificationHandler.prototype.
		...
		if (workingJsonData.content.inPlace) {
			...
			if (!(this.visibleWithoutScrolling_()) && newNotificationsData) {
				...
				for (var i in trivialNotificationsId) {
					...
					$('#sep_notification_' + notificationId,
	...
	$.sep.controllers.NotificationHandler.prototype.
		...
		for (var levelId in workingNotificationsData.content.inPlace) {
			for (var notificationId in
				...
				var element = $('#sep_notification_' + notificationId);
				if (element.length > 0) {
		...
		if (notificationsId.length) {
			this.trivialTimer_ = setTimeout(function() {
				for (var notificationId in notificationsId) {
					var $notification = $('#sep_notification_' +
