...
$.sep.controllers.listbuilder = $.sep.controllers.listbuilder || {};
...
(function($) {
	 ...
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler =
			function($listbuilder, options) {
		this.parent($listbuilder, options);
	};
	$.sep.classes.Helper.inherits($.sep.controllers.listbuilder.ListbuilderHandler,
	 ...
	 * @type {?number}
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.
			sourceType_ = null;
	 ...
	 * @inheritDoc
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.initialize =
			function(options) {
	 ...
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.getSaveUrl_ =
			function() {
	 ...
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.getSaveFieldName_ =
			function() {
	 ...
	 * @protected
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.closeEdits =
			function() {
	 ...
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.save =
			function() {
	 ...
	 * @inheritDoc
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.getEmptyElement =
			function($element) {
	 ...
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.addItemHandler_ =
	 ...
	 * @return {boolean} Should return false to stop event processing.
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.deleteItemHandler_ =
			function(callingContext, opt_event) {
		...
		var $deletions = $callingContext.closest('.sep_controllers_listbuilder')
				.find('.deletions');
	...
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.
		...
		if (jsonData !== false) {
			...
			if (this.sourceType_ == $.sep.cons.LISTBUILDER_SOURCE_TYPE_SELECT) {
				this.disableControls();
		...
		if (jsonData !== false) {
			...
			for (i = 0, limit = $selectInput.length; i < limit; i++) {
				...
				for (j in jsonData.content[i]) {
					...
					if (j == $.sep.cons.LISTBUILDER_OPTGROUP_LABEL) {
						continue;
					}
					...
					if (typeof(jsonData.content[i][j]) == 'object') {
						...
						if (jsonData.content[i][$.sep.cons.LISTBUILDER_OPTGROUP_LABEL] ===
								undefined) {
							continue;
						}

						if (typeof(
								jsonData.content[i][$.sep.cons.LISTBUILDER_OPTGROUP_LABEL]) !=
								'object') {
							continue;
						}

						var label =
								jsonData.content[i][$.sep.cons.LISTBUILDER_OPTGROUP_LABEL][j];
						if (!label) {
							continue;
						}
	 ...
	 * @return {boolean} Should return false to stop event processing.
	 */
	$.sep.controllers.listbuilder.ListbuilderHandler.prototype.editItemHandler_ =
			function(callingContext, opt_event) {
	 ...
	 * @private
	 *
	 * @param {$.sep.controllers.form.AjaxFormHandler} callingForm The form
