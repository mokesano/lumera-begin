# CLA Dataverse Plugin

The [Dataverse Network Project](http://thedata.org/) and the [Sangia Editorial Project](http://lumera.sangia.org/)  are
 partnering to develop plugin that adds data sharing and preservation to the [Code Lumera Editorial](http://lumera.sangia.org/cla/)
 publication process. For more information about the project, visit http://projects.iq.harvard.edu/cla-dvn/about-project.
...
for an overview of data publication workflows supported by the plugin in CLA.
...
The Dataverse plugin is included in CLA 2.4.4. Enable the plugin as usual and configure settings according to the [Dataverse Plugin Guide](https://docs.google.com/document/d/1QgxtxMaWdSZ8gI3wHDkE5EfP4W3M2Za-4DhmX_x3pY0/edit?disco=AAAAAGd77n8#).
...
included in CLA 2.4.3. 

From the CLA install directory, test the patch with: 

`patch --dry-run -d lib/sep/lib/swordappv2 < plugins/generic/dataverse/swordappv2.diff`

If the dry run indicates the patch will apply cleanly, run:

`patch -d lib/sep/lib/swordappv2 < plugins/generic/dataverse/swordappv2.diff`
...
If the SWORD plugin is present in your CLA install, at `plugins/generic/sword`, remove it.
In CLA versions 2.4.2 and earlier, the SWORD plugin uses an incompatible version of the swordapp PHP library and **the Dataverse plugin can't be
installed unless it's removed**. 

#### CLA 2.4.0, 2.4.1, 2.4.2
...
**Please make sure the web server is able to write to the `plugins` and `lib/sep/plugins` directories (including subdirectories).** 
...
* Move the SWORD library files from `plugins/generic/dataverse/lib/swordappv2` to 'lib/sep/plugins/generic/dataverse/swordappv2`
* From the CLA install directory, run `php tools/dbXMLtoSQL.php -schema execute plugins/generic/dataverse/schema.xml` 
