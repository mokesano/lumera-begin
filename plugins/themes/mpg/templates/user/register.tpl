...
{/if}{* !$implicitAuth *}
    ...
    <div class="c-form-field">
        ...
        {if $privacyStatement}<a class="action" href="#privacyStatement" style="float:right">{translate key="user.register.privacyStatement"}</a>{/if}
        {if !$existingUser}
        {fieldLabel name="email" required="true" key="user.email" class="c-form-field__label"}
        <input class="c-form-field__input" type="email" id="signup-email" name="email" value="{$email|escape}" size="30" maxlength="90" /></div>
        {/if}{* !$existingUser *}
            
	<div class="c-form-field">		
		{fieldLabel name="confirmEmail" required="true" key="user.confirmEmail" class="c-form-field__label"}
		<input type="text" id="confirmEmail" name="confirmEmail" value="{$confirmEmail|escape}" size="30" maxlength="90" class="c-form-field__input" /></div>
