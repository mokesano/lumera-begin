...
{else}
	...
	<div id="topBar">
        ...
        <table width="100%" style="border-bottom:1px solid #ccc"><tbody><tr>
            ...
            <td width="12%" bgcolor="#fff" style="padding-top:0em">
                <div id="coverArticleIssue" class="text-img"><a href="{url page="issue" op="view" path=$issue->getBestIssueId($currentJournal)}" class="file"><img class="u-font-sans" src="{$publicFilesDir}/{$homepageImage.uploadName|escape}/homepageImage_en_US.jpg" {if $homepageImageAltText != ''}alt="{$homepageImageAltText|escape}"{else}alt="{$currentJournal->getLocalizedTitle()|strip_tags|escape}"{/if} /></a></div>
                {if $coverPagePath}
                <div id="articleCoverImage" class="text-img"><a href="{url page="issue" op="view" path=$issue->getBestIssueId($currentJournal)}" class="file"><img class="u-font-sans" src="{$coverPagePath|escape}{$coverPageFileName|escape}"{if $coverPageAltText != ''} alt="{$coverPageAltText|escape}"{else} alt="{translate key="article.coverPage.altText"}"{/if}{if $width} width="{$width|escape}"{/if}{if $height} height="{$height|escape}"{/if}/></a>
                </div>
                {/if}
                {call_hook name="Templates::Article::Article::ArticleCoverImage"}</td>
            </tr>
        </tbody>
    </table>
...
{/if}
    ...
    <div id="Declaration" class="Declaration">
        ...
        <div id="PublisherName" class="PublisherName">
            ...
            <div class="statePublisher u-font-serif">{if $currentJournal->getSetting('publisherInstitution')}{$currentJournal->getSetting('publisherInstitution')|escape}{else}Stipwunaraha Research Media{/if} remains neutral with regard to jurisdictional claims in published maps and institutional affiliations.</div>
        </div>
    ...
    <div id="References" class="u-font-serif References">
    ...
    <div id="additionalNotes" class="additionalNotes">
        ...
        <ul class="c-bibliographic-information__list">
            ...
            <li class="c-bibliographic-information__item">
                ...
                <span class="c-bibliographic-information__value u-font-sans">{if $pubIdPlugin->getResolvingURL($currentJournal->getId(), $pubId)|escape}<a id="pub-id::{$pubIdPlugin->getPubIdType()|escape}" title="Permanent link for this article" href="{$pubIdPlugin->getResolvingURL($currentJournal->getId(), $pubId)|escape}">{$pubIdPlugin->getResolvingURL($currentJournal->getId(), $pubId)|escape}</a>{else}{$pubId|escape}{/if}</span>
            </li>{/if}
            
            <li class="c-bibliographic-information__item">
                <h5 class="strong u-font-serif">{translate key="rt.metadata.dublinCore.publisher"} Name</h5>
                <span class="c-bibliographic-information__value u-font-serif">{if $currentJournal->getSetting('publisherInstitution')}{$currentJournal->getSetting('publisherInstitution')|escape}{else}Stipwunaraha Research Media{/if}</span>
            </li>            
        </ul>
        
        {if $pubId}
        <div class="c-article__heading">
            <h4 class="c-article__sub-heading u-font-serif">Article Metrics</h4>
            <ul class="article-metrics">
                <li class="article-metrics__item">
                    {if $pubId}
                    <div data-badge-popover="right" data-badge-type="donut" data-doi="{$articleDOI|escape}" class="altmetric-embed embed" target="_blank" ></div>
                    {else}
                    <div data-badge-popover="right" data-badge-type="donut" data-doi="" class="altmetric-embed embed" target="_blank"></div></span>
                    {/if}                    
                </li>
                <li class="article-metrics__item">
                    <span class="__dimensions_badge_embed__ embed" data-doi="{$pubId|escape}" data-style="small_circle"></span><script async src="https://badge.dimensions.ai/badge.js" charset="utf-8"></script>                    
                </li>
            </ul>
        </div>
        {/if}    
    ...
    <div id="comment" class="comment">
        <h6 class="sub-title u-font-serif">{translate key="article.comments"}</h6>
