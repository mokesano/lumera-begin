...
{if $article->getSubject(null)}{foreach from=$article->getSubject(null) key=metaLocale item=metaValue}
	{foreach from=$metaValue|explode:"; " item=dcSubject}
		{if $dcSubject}
			<meta name="DC.Subject" xml:lang="{$metaLocale|String_substr:0:2|escape}" content="{$dcSubject|strip_tags|escape}"/>
		{/if}
	{/foreach}
{/foreach}{/if}
...
{/foreach}
    ...
    <meta name="DC.Publisher" content="{$currentJournal->getSetting('publisherInstitution')|escape}"/>
