...
<table class="listing" width="100%">
	...
	<tr class="heading" valign="bottom">
		...
		<td width="7%">{sort_heading key="submissions.sec" sort="section"}</td>
		<td width="58%">{sort_heading key="article.title" sort="title"}</td>
		<td width="10%">{sort_heading key="submission.review" sort="review"}</td>
		<td width="10%">{sort_heading key="submission.editorDecision" sort="decision"}</td>
	</tr>
	<tr><td colspan="6" class="headseparator">&nbsp;</td></tr>
{iterate from=submissions item=submission}
	...
	<tr valign="top">
		...
		<td>{if !$submission->getDeclined()}<a href="{url op="submission" path=$reviewId}" class="action">{/if}{$submission->getLocalizedTitle()|strip_unsafe_html}{if !$submission->getDeclined()}</a>{/if}</td>
		<td>
