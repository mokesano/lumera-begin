...
<div id="editorialTeam" class="block member listAddMenu">
{if count($editors) > 0}
	...
	<ul class="editorialTeam">
		{foreach from=$editors item=editor}
			<li><a href="{url op="editorialTeamBio" path=$editor->getId()}"><h4>{$editor->getFullName()|escape}</h4></a>{if $editor->getLocalizedAffiliation()}, {$editor->getLocalizedAffiliation()|escape}{/if}{if $editor->getCountry()}{assign var=countryCode value=$editor->getCountry()}{assign var=country value=$countries.$countryCode}, {$country|escape}{/if}</li>
...
{if count($layoutEditors) > 0}
	...
	<ul class="editorialTeam">
		{foreach from=$layoutEditors item=layoutEditor}
			<li><a href="{url op="editorialTeamBio" path=$layoutEditor->getId()}">{$layoutEditor->getFullName()|escape}</a>{if $layoutEditor->getLocalizedAffiliation()}, {$layoutEditor->getLocalizedAffiliation()|escape}{/if}{if $layoutEditor->getCountry()}{assign var=countryCode value=$layoutEditor->getCountry()}{assign var=country value=$countries.$countryCode}, {$country|escape}{/if}</li>
			<div id="profilePicContent" style="float: right;">
			{assign var="profileImage" value=$user->getSetting('profileImage')}
			{if $profileImage}
			    <img height="{$profileImage.height|escape}" width="{$profileImage.width|escape}" alt="{translate key="user.profile.profileImage"}" src="{$sitePublicFilesDir}/{$profileImage.uploadName}" />
...
{if count($copyEditors) > 0}
	...
	<ul class="editorialTeam" class="block">
		{foreach from=$copyEditors item=copyEditor}
			<li><a href="{url op="editorialTeamBio" path=$copyEditor->getId()}">{$copyEditor->getFullName()|escape}</a>{if $copyEditor->getLocalizedAffiliation()}, {$copyEditor->getLocalizedAffiliation()|escape}{/if}{if $copyEditor->getCountry()}{assign var=countryCode value=$copyEditor->getCountry()}{assign var=country value=$countries.$countryCode}, {$country|escape}{/if}</li>
			<div id="profilePicContent" style="float: right;">
			{assign var="profileImage" value=$user->getSetting('profileImage')}
			{if $profileImage}
			    <img height="{$profileImage.height|escape}" width="{$profileImage.width|escape}" alt="{translate key="user.profile.profileImage"}" src="{$sitePublicFilesDir}/{$profileImage.uploadName}" />
