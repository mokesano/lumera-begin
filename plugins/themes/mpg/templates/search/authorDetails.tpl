...
<div id="article__list">
    <h3>{$publishedArticles|@count} {translate key="article.articles"}</h3>
</div>
<br />
...
{foreach from=$publishedArticles item=article}
	...
	<div id="articleList">
	<ul id="articleInfoList" class="tocMenuArticle"><li id="articleInfoList" class="tocMenuArticle">{$section->getLocalizedTitle()|escape}</li>
        </ul>
	...
	<ul id="author-article-InfoList" class="tocMenuArticle">
		...
		{foreach from=$article->getGalleys() item=galley name=galleyList}<a href="{url journal=$journal->getPath() page="article" op="view" path=$article->getBestArticleId()|to_array:$galley->getBestGalleyId($journal)}" class="file">{$galley->getGalleyLabel()|escape} <span class="fileSize">({$galley->getNiceFileSize()|strip_tags|escape})</span></a></li>
		{/foreach}
		{/if}
	{/if}</ul>
	</div>
