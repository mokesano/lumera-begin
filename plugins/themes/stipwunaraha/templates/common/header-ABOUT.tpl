    ...
    <nav class="journal-navigation">
    	<div class="row">
    		<ul class="column">
    			...
    			<li class="menu-item--current  menu-item--current menu-item--current-parent"><a href="{url page="about" op="editorialTeam"}" >{translate key="navigation.about"}</a></li>
...
<div class="live-area-wrapper">
	...
	<div class="sidebar">
		<div class="column medium-2">
			...
			<div class="live">					
			    <ul >
			        <li {if $headerAbout == $editorialTeam}class="menu-item--current"{else}class=" "{/if}><a href="{url page="about" op="editorialTeam" anchor=""}">{translate key="about.editorialTeam"}</a></li>

			        {if $peopleGroups}
			        	{iterate from=peopleGroups item=peopleGroup}
			        		<li class=" "><a href="{url op="displayMembership" path=$peopleGroup->getId()}">{$peopleGroup->getLocalizedTitle()|escape}</a></li>
			        	{/iterate}
			        {/if}

			        <li {if $setupStep == 2} class="menu-item--current"{/if}><a href="{url page="about" op="displayMembership" path="3"}">2017-2020 Editorial Team</a></li>

			        {call_hook name="Templates::About::Index::Other"}

			        {if $enableAnnouncements}<li class=" "><a href="{url page="announcement"}">News</a></li>{/if}{* enableAnnouncements *}

			        {if $currentJournal->getSetting('membershipFee')}<li {if $headerAbout == memberships}class="menu-item--current"{else}class=" "{/if}><a href="{url page="about" op="memberships"}">{translate key="about.memberships"}</a></li>{/if}

			        {if not ($currentJournal->getSetting('publisherInstitution') == '' && $currentJournal->getLocalizedSetting('publisherNote') == '' && $currentJournal->getLocalizedSetting('contributorNote') == '' && empty($journalSettings.contributors) && $currentJournal->getLocalizedSetting('sponsorNote') == '' && empty($journalSettings.sponsors))}<li class=" "><a href="{url page="about" op="journalSponsorship"}">{translate key="about.journalSponsorship"}</a></li>{/if}

			        {if $currentJournal->getLocalizedSetting('history') != ''}<li class=" "><a href="{url op="history"}">Journal {translate key="about.history"}</a></li>{/if}

			        {if $publicStatisticsEnabled}<li class=" "><a href="{url op="statistics"}">{translate key="about.statistics"}</a></li>{/if}
...
<section id="content" class="article-body">
