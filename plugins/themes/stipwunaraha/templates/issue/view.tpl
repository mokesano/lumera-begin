...
{if !$showToc && $issue}
	...
	{if $coverPagePath}
	<div id="issueCoverImage"><a href="{$currentUrl}"><img class="journal-cover__image" {if $coverPageAltText != ''} title="Cover issue {$coverPageAltText|escape}"{else} title="Cover issue {translate key="issue.coverPage.altText"}"{/if} {if $coverPageAltText != ''} alt="{$coverPageAltText|escape}"{else} alt="{translate key="issue.coverPage.altText"}"{/if}{if $width} width="{$width|escape}"{/if}{if $height} height="{$height|escape}"{/if} {if $coverPagePath} src="{$coverPagePath|escape}{$issue->getFileName($locale)|escape}"{/if} style="margin-top: 0" width="100%" /></a>
		{else}
		<div id="issueCoverImage"><a href="{$currentUrl}"><img class="journal-cover__image cover-lazy img-default" title="Cover issue default" src="{$publicFilesDir}/{$homepageImage.uploadName|escape}/homepageImage_en_US.jpg" alt="Cover issue default" style="margin-top: 0" width="100%" />
	</div>
	{/if}
...
{elseif $issue}
	...
	{if $issueGalleys}
		<h3 class="title-issue">{translate key="issue.fullIssue"}</h3>
    ...
    <h3 class="issue">In this issue <span class="articleCount">({$issue->getNumArticles()|escape} {translate key="article.articlesCount"})</span></h3>
