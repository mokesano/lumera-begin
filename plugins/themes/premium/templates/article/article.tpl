...
{else}
    ...
    <div id="author" class="authorName">
        ...
        {assign var=authorCount value=$authors|@count}
            ...
            {if $author->getData('orcid')}<a title="View ORCID ID author profile" href="{$author->getData('orcid')|escape}" target="_blank" class="icon"><img src="{$baseUrl}/public/site/images/orcid_16x16.svg" style="height:12px" alt="orcid" /></a>{esle}{/if}
            {if $author->getUrl()}<a title="View Google Scholar author profile" href="{$author->getUrl()|escape}" target="_blank" class="icon"><img src="{$baseUrl}/public/site/images/scholar.svg" style="height:14px" alt="scholar" /></a>{else}{/if}{if $i==$authorCount-2} and {elseif $i<$authorCount-1}, {/if}{/if}{/if}
        {/foreach}
        </div>
    ...
    <button onclick="myFunction()" id="myBtn" class="show-hide-details u-font-sans">Show more</button>    
    <script>
        function myFunction() {
    ...
    <div class="rightandpermission">
        <div class="articlepubDOI">
            <div id="DOI" class="pubDOI">
                ...
                <ul class="info" style="float: right;"><li id="permission"><a href="#copyright">Get rights and content</a></li></ul>
            </div>
        </div>
    ...
    {foreach from=$metaValue|explode:"; " item=dcSubject}
        {if $dcSubject}
            <span class="subjectId--value">{$dcSubject|strip_unsafe_html|nl2br}</span>
        {/if}
    {/foreach}
    {/foreach}{/if}
        </div>
    ...
    {if $journalRt->getSupplementaryFiles() && is_a($article, 'PublishedArticle') && $article->getSuppFiles()}
        <h6>{translate key="rt.suppFiles"}</h6>
        {foreach from=$article->getSuppFiles() item=suppFile key=key}
        <div class="supplement-files--value">
            <h4 class="supplement-files--label"><a href="{url page="article" op="downloadSuppFile" path=$article->getBestArticleId()|to_array:$suppFile->getBestSuppFileId($currentJournal)}" class="file">{translate key="article.additionalFiles"} {$key+1}:</a> {if $suppFile->getSuppFileTitle()}<span class="fileClick">{$galley->getViews()}</span>{/if}</h4>
            <span class="supplement-files--label"><b class="strong">{if $suppFile->getSuppFileTitle()}{$suppFile->getSuppFileTitle()|escape}</b>.{/if}
